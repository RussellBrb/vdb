...
**Motion behavior layer (v14):** `motion` also accepts behavior-driven parts while keeping `frames` as the backward-compatible escape hatch. Part additions: `shape?:'circle'`, `r?`, `pivot?:[x,y]`, `parent?:partId`, `labelX?`, `labelY?`, `behaviors?:[{ type:'rotate', deg?, period?, phase?, ease?, segmented? } | { type:'oscillate', axis:'x'|'y', amp?, period?, phase?, ease?, segmented? } | { type:'pulse', prop:'opacity', period?, phase? } | { type:'flow', path:[[x,y],...], period?, phase?, ease? } | { type:'swing', deg? } | { type:'stretch', scale? } | { type:'lift', lift?, riseFraction?, dwellFraction? } | { type:'dash', offset?, direction? }]`. Component-level `period`, `speed`, and `phase` provide one shared mechanism phase; behavior-level values can override. Behaviors compile to CSS `@keyframes`; all use `transform`/`opacity` except the `fluid-flow` dash primitive, which uses `stroke-dashoffset`. No rAF, SMIL, solver, or dependency. Presets: `gear`, `piston`, `lever`, `cam`, `rack-pinion`, `spring`, `fluid-flow`, `toggle-switch`, and `valve`. Step snapshots use `params.stepParam`/`params.steps`; boolean toggle presets use `params.param` against existing `state`/`toggle` controls.
