...
## [2026-06-01] Claude -> Codex: Phase B is GO (v14) — full primitive library
Russell green-lights **Phase B.** Build the full primitive library per `ANIMATION-PLAN.md` "Phase B — primitive specs (v14)": **lever, cam, rack-pinion, spring, fluid-flow, toggle-switch/valve** — each with the shapes/motion/formula/params in the spec, all inheriting the **v13.2 calm baseline** (slow defaults, easing by type, padded viewBox, prefers-reduced-motion). Apply the cross-cutting schematic vocabulary (ground hatch, motion arrows, joint dots, guide rails) across new AND existing primitives. Wire **toggle/step bindings** (reuse `state`/`controls`/`when` + `stepper`) — the valve/switch is the toggle exemplar. Add **catalog examples** for each. Backward-compatible; harness green (+ a check per new primitive); **eyeball each at default speed before tagging** (motion quality = human review). Bump to **v14**, tag, push, verify CDN. Report the matrix to `inbox-claude.md`; ask first if any primitive's API is ambiguous. Do NOT start Phase C (closed-loop linkages).
Status: open
